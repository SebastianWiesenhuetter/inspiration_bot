<template>
    <div class="h-screen w-screen">
        <div ref='threejsMap' @click="onClick" @mousemove="onDrag" @mousedown="isMouseDown = true" @touchstart="isMouseDown = true" @mouseup="mouseUp" @touchend="mouseUp">
            <div class="absolute top-2 w-full flex flex-row justify-between space-x-1">
                <button class=" h-10 w-full bg-slate-200 rounded-lg" @click.stop="sceneMode = 'gallery'">gallery</button>
                <button class=" h-10 w-full bg-slate-200 rounded-lg" @click.stop="sceneMode = 'analyse'">analyse</button>
                <button class=" h-10 w-full bg-slate-200 rounded-lg" @click.stop="sceneMode = 'generate'">generate</button>
            </div>
        </div>
    </div>
</template>
...
const loadGallery = async () => {
    ...
    const offset = ref(0)
    await fetch('/image_resolutions.json')
        ...
        .then(data => {
            data.forEach((image: ImageData) => {
                ...
                plane.visible = true
                plane.translateX(offset.value)
                plane.userData = { image }
                plane.userData.isImage = true
                galleryGroup.add(plane);
            });
        });
    arrangeIn2DGrid(3, 3, 0.1, galleryGroup.children)
