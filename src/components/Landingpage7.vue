<template>
  <div ref="canvasContainer" class="h-screen w-screen relative">
    ...
    <div class="absolute top-0 right-0 m-4" style="pointer-events: none;">
      <p style="pointer-events: auto; background: rgba(255, 255, 255, 0.8); padding: 5px; border-radius: 5px;">Loaded Images: {{ loadedImagesCount }}</p>
    </div>
  </div>
</template>
...
//const maxTextureSize = renderer.capabilities.maxTextureSize; // to be deleted
//console.log(maxTextureSize); // to be deleted
...
const Y_COORD_MULTIPLIER = 30000; // Example large number 80000 was also good
...
const loadImagesFromCSV = async (csvPath: string, n: number) => {
  ...
  //const basePath = '/sample_images_10k_orig01_128/';
  const basePath = '/sample_images_10k_orig01_64/';
...
async function initGallery() {
  ...
  //const csvPath = '/tsne_coordinates_pca_r0_2d_V2_res128.csv';
  const csvPath = '/tsne_coordinates_pca_r0_2d_V2_res64.csv';
  const imageData = await loadImagesFromCSV(csvPath, 5000);
